...
class RTMClient {
     ...
     */
    translate(text, src, dst, type, profanity, timeout, callback) {
        let cmd = 'translate';
        let ts = FPManager.instance.timestamp;
        let salt = RTMClient.MidGenerator.gen();
        let sign = genSign.call(this, salt, cmd, ts);
     ...
     * @callback
     * @param {Error}   err
     * @param {object(text:string)}  data
     */
    profanity(text, action, timeout, callback) {
        let cmd = 'profanity';
        let ts = FPManager.instance.timestamp;
        let salt = RTMClient.MidGenerator.gen();
        let sign = genSign.call(this, salt, cmd, ts);
     ...
     */
    transcribe(audio, lang, action, timeout, callback) {
        let cmd = 'transcribe';
        let ts = FPManager.instance.timestamp;
        let salt = RTMClient.MidGenerator.gen();
        let sign = genSign.call(this, salt, cmd, ts);
...
RTMClient.MidGenerator = class {
    ...
    static strBuilder = [];
    static gen() {
        let count = RTMClient.MidGenerator.count;
        let sb = RTMClient.MidGenerator.strBuilder;

        if (++count > 999) {
            count = 1;
        }

        RTMClient.MidGenerator.count = count;
        sb.length = 0;
